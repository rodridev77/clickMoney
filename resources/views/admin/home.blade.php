<div class="row">
    <div class="col-lg-3 col-6">
        <!-- small box -->
        <div class="small-box bg-info">
            <div class="inner">
                <h3>150</h3>

                <p>Transaçoes</p>
            </div>
            <div class="icon">
                <i class="ion ion-bag"></i>
            </div>
            <a href="#" class="small-box-footer" onclick="loadViewInSelectContent('{{route('admin.transacao.efetivada')}}')">Ver Mais <i
                    class="fas fa-arrow-circle-right" ></i></a>
        </div>
    </div>
    <!-- ./col -->
    <div class="col-lg-3 col-6">
        <!-- small box -->
        <div class="small-box bg-success">
            <div class="inner">
                <h3>R$ 100<sup style="font-size: 20px">,00</sup></h3>

                <p>Saldo</p>
            </div>
            <div class="icon">
                <i class="fas fa-money-bill-alt"></i>
            </div>
            <a href="#" class="small-box-footer" onclick="loadViewInSelectContent('{{route('admin.saldo')}}')">Ver Mais <i
                    class="fas fa-arrow-circle-right"></i></a>
        </div>
    </div>
    <!-- ./col -->
    <div class="col-lg-3 col-6">
        <!-- small box -->
        <div class="small-box bg-warning">
            <div class="inner text-white">
                <h3>44</h3>

                <p>Transferencia de Saldo</p>
            </div>
            <div class="icon ">
                <i class="fas fa-exchange-alt"></i>
            </div>
            <a href="#" class="small-box-footer" onclick="loadViewInSelectContent('{{route('admin.transferencia')}}')"><span class="text-white">Ver Mais <i
                class="fas fa-arrow-circle-right"></i>
            </span></a>
        </div>
    </div>
    <!-- ./col -->
    <div class="col-lg-3 col-6">
        <!-- small box -->
        <div class="small-box bg-info">
            <div class="inner">
                <h3>65</h3>

                <p>Trocas</p>
            </div>
            <div class="icon">
                <i class="fas fa-sync-alt"></i>
            </div>
            <a href="#" class="small-box-footer" onclick="loadViewInSelectContent('{{route('admin.trocas')}}')">Ver Mais <i
                    class="fas fa-arrow-circle-right"></i></a>
        </div>
    </div>
    
    <div class="container">
          <div class="container">
        <div class="card">
  <h1 class="card-header">Taxas</h1>
  <div class="card-body">
    <h2 class="card-title">Valores de taxa vão ser adicioonados</h2>
    <p class="card-text">With supporting text below as a natural lead-in to additional content.</p>
    <a href="#" class="btn btn-primary">Go somewhere</a>
  </div>
</div>
    </div>
    </div>
    <!-- ./col -->
</div>
<!-- /.row -->
<!-- Main row -->